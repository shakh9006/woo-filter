...
class Query
{
	...
	public function getSql($name_query = 'select', $parametres = null) {
		...
		if($name_query == 'update') {
			...
			return apply_filters('wf_query_update', "UPDATE {$data['table']} {$data['asTable']} {$data['join']} SET {$set} {$data['where']}", $this->model);
		}

		if($name_query == 'delete'){
			$this->select = ($this->select != "*") ? $this->select : '';
			return apply_filters('wf_query_delete', " DELETE {$this->select} FROM `{$data['table']}` \n {$data['asTable']} \n {$data['join']} \n {$data['where']}", $this->model);
		}
		...
		if($name_query == 'count'){
			...
			return apply_filters('wf_query_select', " SELECT {$this->select} FROM `{$data['table']}` \n {$data['asTable']} \n {$data['join']} \n {$data['where']}  \n {$data['group_by']} \n {$data['having']}", $this->model);
		}
